...
loadpath = '/root/qqbot/gocqhttp-1.8/logs'  # 此处填gocq的logs路径
self_id = '3347359415'  # 此处填机器人的QQ号
load_in_path = '/root/qqbot/Hoshino/res'  # 此处填词云图片保存的路径
...
def makeclouds(gid):
    ...
    bot = nonebot.get_bot()
    today = datetime.date.today().__format__('%Y-%m-%d')
    f = open(loadpath + os.sep + f"{today}.log", "r", encoding="utf-8")
    f.seek(0)
    gida = str(gid)
    msg = ''
    ...
    stopwords = set()
    content = [line.strip() for line in open(load_in_path + os.sep + f"tyc.txt", encoding='utf-8').readlines()]
    stopwords.update(content)
    txt = " ".join(ls)
    w = wordcloud.WordCloud(font_path=load_in_path + os.sep + f"SimHei.ttf", \
    ...
    if gid:
        try:
            ...
            os.remove(load_in_path + os.sep + f"{today}-{gid}.png")
            shutil.move(f"{today}-{gid}.png", load_in_path)
    else:
        try:
            shutil.move(f"{today}.png", load_in_path)
        except:
            os.remove(load_in_path + os.sep + f"{today}.png")
            shutil.move(f"{today}.png", load_in_path)
...
def makecloudsb(gid):
    ...
    yesterday = (datetime.date.today() + datetime.timedelta(-1)).__format__('%Y-%m-%d')
    gida = str(gid)
    f = open(loadpath + os.sep + f"{yesterday}.log", "r", encoding="utf-8")
    f.seek(0)
    msg = ''
    for line in f.readlines():  # 删除前缀和自己的发言
        if self_id in line or gida not in line:
    ...
    if gid:
        try:
            ...
            os.remove(load_in_path + os.sep + f"{yesterday}-{gid}.png")
            shutil.move(f"{yesterday}-{gid}.png", load_in_path)
    else:
        try:
            shutil.move(f"{yesterday}.png", load_in_path)
        except:
            os.remove(load_in_path + os.sep + f"{yesterday}.png")
            shutil.move(f"{yesterday}.png", load_in_path)
