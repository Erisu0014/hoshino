...
try:
    with open(_blue_pool_config_file, encoding='utf8') as f:
        _blue_blue_group_pool = json.load(f)
...
gacha_10_aliases = ('档案十连')
gacha_1_aliases = ('档案单抽')
gacha_300_aliases = ('档案井')

@sv.on_fullmatch(('档案查看卡池'))
...
async def gacha_300(bot, ev: CQEvent):
    ...
    msg = [
        f"\n增加了很棒的学生哦！ {res}",
        f"★★★×{up+s3} ★★×{s2} ★×{s1}",
        f"获得UP角色×{up}与档案母猪石×{50*(up+s3) + 10*s2 + s1}！\n第{result['first_up_pos']}抽首次获得up角色" if up else f"获得档案母猪石{50*(up+s3) + 10*s2 + s1}个！"
    ]
...
async def kakin(bot, ev: CQEvent):
    ...
    for m in ev.message:
        if m.type == 'at' and m.data['qq'] != 'all':
            ...
            count += 1
    if count:
        await bot.send(ev, f"已经联系小助理为{count}位用户充值完毕！谢谢惠顾～")
