    ...
    async def update_token(self) -> Union[str, bool]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                req=await session.post(url,data=data,proxy='http://127.0.0.1:41091')
                if req.status != 200:
...
async def user_info(url: str) -> Union[dict, str]:
    try:
        ...
        async with aiohttp.ClientSession() as session:
            req=await session.get(url,proxy='http://127.0.0.1:41091',headers=header)
            if req.status == 401:
...
async def ApiInfo(project: str, url: str) -> Union[dict, str]:
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url,headers=headers,proxy='http://127.0.0.1:41091') as req:
                if req.status == 401:
